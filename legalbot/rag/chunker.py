...
def _count_tokens(text: str) -> int:
    """Count tokens using tiktoken cl100k_base (same as legalbot core)."""
    try:
...
class LegalChunker:
    ...
    # Chinese sentence-boundary punctuation for truncation
    _BOUNDARY_CHARS = frozenset("。！？，；、：""''））】》」")

    def __init__(
        self,
        max_chunk_tokens: int = 600,
        overlap_tokens: int = 100,   # 200→100，减少因overlap过大导致的token超限
        min_chunk_chars: int = 50,
        max_chunk_chars: int = 7800,
    ):
    ...
    def chunk(self, text: str, metadata: dict) -> list[Chunk]:
        ...
        3. 保留上下文重叠（overlap）
        4. 相邻短 chunks 合并
        5. 每个 chunk 携带元数据：law_name, article_no, chapter, law_area
        """
        if not text or not text.strip():
            return []

        # Step 0: Hard-truncate at sentence boundary (DashScope 8192 limit)
        ...
        # Step 4: Drop tiny chunks (< min_chunk_chars)
        before = len(chunks)
        chunks = [c for c in chunks if len(c.text) >= self.min_chunk_chars]
        if len(chunks) < before:
            logger.debug("Dropped {} tiny chunks", before - len(chunks))

        return chunks

    def _truncate_at_sentence_boundary(self, text: str) -> str:
        """Truncate at the last full sentence boundary within max_chunk_chars.

        DashScope text-embedding-v4 has a hard limit of 8192 characters.
        We leave 400 chars of margin (max_chunk_chars = 7800).
        """
        if len(text) <= self.max_chunk_chars:
            return text
        truncated = text[: self.max_chunk_chars]
        # Walk backwards from end to find a sentence-ending punctuation
        for i in range(len(truncated) - 1, int(len(truncated) * 0.6), -1):
            if truncated[i] in self._BOUNDARY_CHARS:
