...
class RAGSearchTool(Tool):
    ...
    def _format_results(query: str, results: list) -> str:
        ...
        for i, r in enumerate(results, 1):
            ...
            chunk_id = r.chunk.id if hasattr(r, "chunk") else f"chunk-{i}"
            ...
            if len(text) > 300:
                lines.append("   ...")
            lines.append(f"   [反馈ID: {chunk_id}]")
            lines.append("")
        lines.append("---")
        lines.append("以上结果对您有帮助吗？")
        lines.append("  有帮助 → 请说：legal_feedback result_id=xxx helpful=true")
        lines.append("  不准确 → 请说：legal_feedback result_id=xxx helpful=false")
        lines.append("  纠正 → 请说：legal_feedback result_id=xxx correction='正确内容'")
        lines.append("  （result_id 即上述结果标注的 [反馈ID]）")
        return "\n".join(lines)
