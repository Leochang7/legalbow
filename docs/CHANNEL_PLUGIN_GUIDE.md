...
Build a custom legalbot channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of legalbot (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

legalbot discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `legalbot gateway` starts, it scans:

1. Built-in channels in `legalbot/channels/`
2. External packages registered under the `legalbot.channels` entry point group
...
```
legalbot-channel-webhook/
├── legalbot_channel_webhook/
...
```python
# legalbot_channel_webhook/__init__.py
from legalbot_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# legalbot_channel_webhook/channel.py
import asyncio
...
from legalbot.config.schema import Base
...
# pyproject.toml
[project]
name = "legalbot-channel-webhook"
version = "0.1.0"
dependencies = ["legalbot", "aiohttp"]

[project.entry-points."legalbot.channels"]
webhook = "legalbot_channel_webhook:WebhookChannel"
...
```bash
pip install -e .
legalbot plugins list      # verify "Webhook" shows as "plugin"
legalbot onboard           # auto-adds default config for detected plugins
```

Edit `~/.legalbot/config.json`:
...
```bash
legalbot channels login <channel_name>
legalbot channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `legalbot onboard`. Override to declare your fields. |
...
Built-in channels use Pydantic config models (subclassing `Base` from `legalbot.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `legalbot.config.schema.Base`:
...
Override `default_config()` so `legalbot onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `legalbot-channel-{name}` | `legalbot-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `legalbot_channel_{name}` | `legalbot_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/legalbot-channel-webhook
cd legalbot-channel-webhook
pip install -e .
legalbot plugins list    # should show "Webhook" as "plugin"
legalbot gateway         # test end-to-end
```

## Verify

```bash
$ legalbot plugins list
