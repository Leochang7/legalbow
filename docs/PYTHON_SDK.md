...
Use legalbot programmatically — load config, run the agent, get results.
...
### `legalbot.from_config(config_path?, *, workspace?)`

Create a `legalbot` from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.legalbot/config.json`. |
...
```python
from legalbot.agent import AgentHook, AgentHookContext
...
async def main():
    bot = legalbot.from_config(workspace="/my/project")
