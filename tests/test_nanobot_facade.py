"""Tests for the legalbot programmatic facade."""
...
from legalbot.legalbot import legalbot, RunResult
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        legalbot.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = legalbot.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with patch("legalbot.config.loader.load_config") as mock_load, \
         patch("legalbot.legalbot._make_provider") as mock_prov:
        mock_load.return_value = Config()
        mock_prov.return_value = MagicMock()
        mock_prov.return_value.get_default_model.return_value = "test"
        mock_prov.return_value.generation.max_tokens = 4096
        legalbot.from_config()
...
async def test_run_hooks_restored_on_error(tmp_path):
    ...
    from legalbot.agent.hook import AgentHook
...
def test_workspace_override(tmp_path):
    ...
    bot = legalbot.from_config(config_path, workspace=custom_ws)
    assert bot._loop.workspace == custom_ws


def test_sdk_make_provider_uses_github_copilot_backend():
    from legalbot.config.schema import Config
    from legalbot.legalbot import _make_provider
...
def test_import_from_top_level():
    from legalbot import legalbot as N, RunResult as R
    assert N is legalbot
